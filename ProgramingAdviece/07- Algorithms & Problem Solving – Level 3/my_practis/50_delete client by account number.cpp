...
void delet_client(string your_account,vector<client>& your_vclient){
    client my_client;
    char answer='y';
    if(search_account(your_account,your_vclient,my_client)){
        print_client_info(my_client);
        cout<<"do you want to delet client:";cin>>answer;
        if(answer=='y'||answer=='Y'){
            make_marke_client(your_vclient,your_account);
            load_data_to_fill(your_vclient,client_fill);
            your_vclient=load_data_from_fill(client_fill);
            cout<<"client delete succusfull\n";
...
int main(){
    ...
    delet_client(account_number,vclient);
